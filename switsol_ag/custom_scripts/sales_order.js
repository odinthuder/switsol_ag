...
frappe.ui.form.on("Sales Order", {
	...
	new_delivery_date: function() {
		cur_frm.set_value("delivery_date", cur_frm.doc.new_delivery_date);
	}
})
...
cur_frm.cscript.custom_refresh = function() {
	...
	if(!cur_frm.doc.delivery_date) {
		cur_frm.set_value("delivery_date", dateutil.add_days(cur_frm.doc.new_transaction_date, 2));
	...
	if(cur_frm.doc.company == "Gilgen Storen AG") {
		...
		if(cur_frm.doc.docstatus == 1) {
			cur_frm.add_custom_button(__('Als montiert markieren'), cur_frm.mark_as_delivered, __("Make"));
			cur_frm.page.set_inner_btn_group_as_primary(__("Make"));
		}
		if(cur_frm.doc.docstatus == 0) {
			if(frappe.session.user != "Administrator" && !cur_frm.doc.clerk_name) {
				if(!cur_frm.doc.clerk_name) {
					cur_frm.set_value("clerk_name", frappe.session.user);
				}
				if(!cur_frm.doc.chief_signature) {
					cur_frm.set_value("chief_signature", frappe.session.user);
				}
				// cur_frm.set_value("employee_signature","")
			}
...
cur_frm.cscript.delivery_period = function() {
	delivery_days = cur_frm.doc.delivery_period * 7;
	cur_frm.set_value("delivery_date", dateutil.add_days(cur_frm.doc.new_transaction_date, delivery_days));
	cur_frm.refresh_field("delivery_date");
}
...
cur_frm.mark_as_delivered = function(frm) {
	cur_frm.set_value("per_delivered", 100);
